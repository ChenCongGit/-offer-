...
// 根据二叉树获得pNode中序遍历下一个节点
BinaryTreeNode* GetNext(BinaryTreeNode* pNode)
{
    // 特殊情况
    if (pNode == nullptr) return nullptr;

    // 如果pNode有右子树，则下一个节点在它右子树的最左子节点
    BinaryTreeNode* p = pNode;
    if (p->m_pRight != nullptr)
    {
        p = p->m_pRight;
        // 从该节点开始下溯，有左节点走左边，有右节点走有边，否则返回当前节点
        while (p != nullptr)
        {
            if (p->m_pLeft != nullptr) p = p->m_pLeft;
            else return p;
        }
    }

    // 如果pNode没有右子树， 则下一个节点在它的祖先节点的另外的分支
    // 从pNode向上回溯，直到到达根节点
    while (p->m_pParent != nullptr)
    {
        // 当p回溯到有右子树的节点时，该节点为它的下一个节点
        if (p->m_pParent->m_pLeft == p) return p->m_pParent;
        p = p->m_pParent;
    }
    return nullptr;
}
